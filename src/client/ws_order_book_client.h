...
public:
    using OnUpdateCallback = std::function<void(double, double, double, double)>;

    WsOrderBookClient(const std::string& uri)
        : m_uri(uri), m_is_connected(false), m_ioc(), m_ssl_ctx(net::ssl::context::tlsv12_client), m_resolver(m_ioc), m_ws(beast::ssl_stream<beast::tcp_stream>(m_ioc, m_ssl_ctx)) {
        std::cout << "Initializing WebSocket client with URI: " << m_uri << std::endl;
    }

    void set_update_callback(OnUpdateCallback callback) {
...
private:
    ...
    void on_message(const std::string& message) {
        ...
        try {
            ...
            if (data.contains("type") && data["type"] == "l2_updates" && data.contains("symbol") && data["symbol"] == "BTCGUSDPERP") {
                ...
                if (m_update_callback) {
                    //std::cout << "Invoking update callback with best bid: " << m_best_bid << ", best bid quantity: " << m_best_bid_quantity
                    //          << ", best ask: " << m_best_ask << ", best ask quantity: " << m_best_ask_quantity << std::endl;
                    m_update_callback(m_best_bid, m_best_bid_quantity, m_best_ask, m_best_ask_quantity);
                }
            }
        } catch (const std::exception& e) {
    ...
    void trim_and_update_best_levels() {
        ...
        if (!m_bid_orders.empty()) {
            ...
            m_best_bid = -std::numeric_limits<double>::infinity();
        ...
        if (!m_ask_orders.empty()) {
            ...
            m_best_ask = std::numeric_limits<double>::infinity();
    ...
    OnUpdateCallback m_update_callback;
    std::mutex m_mutex;
